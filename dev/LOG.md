...
- 预注册、实现与测试在正式结果前由 commit `8c1cc4b` 提交并推送；路径修复由 commit `deb6e7c` 独立提交并推送。正式 artifact 记录 clean-tree commit `deb6e7c84016044b08c5af0062dfd921e6914aea`。
- 首次正式调用在模型/结果计算前 fail-closed：冻结 manifest 保留 WSL `/mnt/d/...z8` 绝对路径，本机缺少原始游戏二进制，`_manifest_provenance` 抛出 `FileNotFoundError`，wall=`0.84s`，未产生结果 JSON。CRWM-1 是 frozen cache 的离线 no-oracle 审计，不执行 live interpreter；故只把复验方式改为逐 split 核对本地 `manifest.json / episodes.jsonl / token_events.txt` 与 SG22R reference 已冻结 SHA，并记录本轮不重验 `.z8`。cache/reference/model/门槛/信息屏障均不变；修复单独提交后再运行。

### 正式结果与证据

- 命令：`python3 experiments/e3_crwm1_no_oracle_candidates.py --threads 4 --output results/e3_scan/e3_crwm1_no_oracle_candidates.json`；外部 `/usr/bin/time -p` wall=`1.20s`。
- 环境：macOS arm64，Python `3.9.6`，PyTorch `2.8.0`，CPU，threads=`4`；closed-form fit deployment wall=`0.04477s`，`640` train examples 压成 `443` prototypes。
- 产物：`results/e3_scan/e3_crwm1_no_oracle_candidates.json`，`95,298 bytes`，SHA-256 `64ca1bddaac177e9aea51831ea586b14a73b2fb34807726d37bda132415a56de`。
- DATA：三 split 的 manifest/episodes/token-events SHA 全等于 SG22R reference；cache/reference SHA、tree `8/40/160/616`、graph 四项 no-leak、constraint 与 vocabulary/action-order identity 全 PASS。

| 冻结 gate | 门槛 | 结果 | 判定 |
|---|---:|---:|---|
| FLOW | oracle proposal=`0`；teacher context=`0` | `0 / 0`；160 groups 均先固化 proposal fingerprint 后读 evaluator target | **PASS** |
| candidate precision | `>=.95` | `616/616 = 1.000` | **PASS** |
| candidate recall | `>=.95` | `616/616 = 1.000` | **PASS** |
| per-first candidate-set exact | `>=.95` | `160/160 = 1.000`，含8个 terminal empty-set | **PASS** |
| invalid transition rate | `<=.05` | `0/616 = 0` | **PASS** |
| no-oracle self second exact | `>=.95`，分母为 proposed∪official | `616/616 = 1.000` | **PASS** |

first projection 分解为 stationary hold `88`、plan constraint `32`、known edge `32`、terminal `8`；false positive=`0`、false negative=`0`、premature self-context failure=`0`、error record=`0`。first proposal+transition p50/p95=`0.1365/0.1425 ms`，second transition p50/p95=`0.1081/0.1130 ms`；本轮 timing 只作同机资源记录，不设速度胜负门。

### 观察、解释与决定

- **观察**：SG22R 的 perfect two-step 结果并不依赖 evaluator 提供第二步 candidate set。projected next-affordance mask 在 160 个 first branches 上自主恢复完整 616 个合法候选，且 self-routed transition 保持 616/616 exact。
- **解释**：在 frozen seventh-fresh 两步域内，“past-observation graph + public-plan constraint + learned residual”已经同时承担 candidate generation 与 transition scoring；之前保留的 candidate-oracle 边界可以在该域内关闭。
- **仍未证明**：评分时仍使用 frozen official tree targets；没有 live interpreter、horizon 8/32、规则冲突/撤销、uncertainty calibration、pure-neural matched baseline 或第二环境。本结果不能外推为 CRWM H1/H2/H3，也不能晋级 `main`。
- 机器 verdict 原样为 **`PHASE1_GO_LONG_HORIZON_REQUIRED`**。下一步 CRWM-2 必须进入 live horizon `{2,8,32}`，统一比较 pure neural、retrieval、constraint-only 与 compiled+residual，并把非法转移、成功率、ECE/Brier、延迟和冲突恢复全部纳入冻结判官。
