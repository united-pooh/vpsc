...
承接 d4/SG29：d4 MoE-SNN 质量 超 Transformer（5.641 vs 5.845）但**不省计算**——`scaling_variants.py:114` 确认三专家全部执行后加权，top-k=1 仍先算完所有专家再乘零。用户提出真条件计算方向：**预测自由能驱动的分块硬路由 + 未激活专家惰性衰减**，且要求验证"早期自由能主线能否与当前 gated-trace 主线结合"。本条是理论设计与实验骨架，不写代码。

### 验证（针对源码，非用户转述）

- **d4 dense 属实**：`_MoEGatedTraceCore.forward` 对全部专家 `exp(x)` 后 `_combine` 加权；topk 分支仅改权重不改执行。d4 改善质量不省算力。
- **惰性衰减数学精确**：`portable_gated_trace._gated_trace` 递推 `state = decay·state + injection·writes`，writes=0 时 `z_{t+Δt}=d^Δt z_t` 逐元素一次乘法恢复（d 逐状态、时间恒定）。跳过专家不需跑 kernel——该特性成立且实现零成本。
- **块内 scan 保留**：B-token 块内 affine scan 深度 O(log B)；`forward_multi_query_eligibility` 已支持稀疏 query（strictly increasing indices），块级读出已可行。
- **两主线当前断裂**：`free_energy.py`/`VPSCNet.total_free_energy`（Theorem 2 的 F：逐层预测误差 `x_l − W_down x_{l+1}` + class prior 顶）属 B-stage mean-field net；`E3GatedTraceScanCore` grep 无 F/W_down/prediction error。当前 LM 训练（sg28/sg29）纯 CE。**早期 F 线与 gated-trace 线无共享 F 计算**——"F 作路由信号"需在 E3 上**重新定义 F**，非自动产生。

### F 的两条定义线（用户要求都试）

- **F-PC 线（重构式，接 Theorem 2）**：在 E3 加 `W_down`，`F_block = ‖x_block − W_down x_upper_block‖²/σ²`（复刻 `VPSCNet.layer_free_energy` 结构）。重接早期 PC F，并修复 S1 指出的容量洞（byte-LM 中 49k `W_down` grad=None）——F 由此同时作路由信号 + 辅助训练目标。代价：新增 W_down 参数 + 顶层 mu 定义。
- **F-inertial 线（惯性延续，无 W_down）**：`F_block = ‖z_t − d z_{t-1}‖²`（"预测"= 惯性延续，误差=写入量）。无新参数、与 gated-trace 原生贴合，但非 Theorem 2 严格 F（无自上而下预测）。更便宜更弱。

两线独立实现、独立判官，不混用 F 定义。

### 方向（creative-ai 五分支，FE-3 为目标态、FE-1 为首发切片）

| # | 方向 | F 整合 | scan 保留 | T4 实测加速可能 |
|---|---|---|---|---|
| FE-1 | 块硬 top-1/2，q_φ 预测 F̂，惰性衰减（首发） | 需 E3 加 F（两线任选） | 块内 O(log B) | 中（小模型路由开销或抵消） |
| FE-2 | F 即写门（合 d7）：逐神经元预测误差直接门写入，无宏观专家 | F 即门 | 全序列 scan 但稀疏写 | 低-中（稀疏写需 kernel 才加速） |
| FE-3 | FE-1+FE-2：逐神经元 F 门写 + 块级 F 路由时间尺度（目标态） | 完全整合 | 保留 | 最高（理论） |
| FE-4 | Early-exit/按需精修（快专家+慢精修专家） | F 决定是否精修 | 保留 | 中 |
| FE-5 | 纯低精度（FP8/INT8）SNN 无路由 | 无 | 保留 | 与 Transformer 差距不明（用户已排除） |

**按用户决定：逐个来，FE-1 先行。** FE-1 内 F 用哪条线——两线都试（FE-1a=F-PC, FE-1b=F-inertial），独立判官。

### FE-1 架构（理论设计）

对第 k 个 token 块（块长 B∈{32,64}）：
1. 廉价路由预测头 `q_φ(block stats, prev energy, router state) → F̂_{k,e}`（不执行专家，预测各专家处理该块的 F）。
2. `e* = argmin_e F̂_{k,e}`（或 top-2 当 ΔF̂=F̂₍₂₎−F̂₍₁₎ 小）。
3. 只运行被选专家：`z^{e*}_k = AffineScan_{e*}(x_k)`。其余专家**完全跳过**（无 input proj / content / scan / output proj）。
4. 未激活专家状态惰性衰减：记录 (last_state, last_pos, decay)；再激活时 `z ← d^Δt z` 一次恢复。

训练稳定性：dense warm-up → 真实 F 监督 q_φ → soft→top-2→top-1 退火 → 负载均衡（专家使用熵 ≥ log E − 1，每专家 ≥5% 块）。训练后期也可只跑被选专家，训练算力亦降。

### 重要边界（诚实）

- **T4 小模型路由开销风险**：块路由（q_φ + 调度 + 非连续专家状态）或抵消 3 小专家的节省，"FLOPs 少了运行反慢"为大概率非边缘。速度结论或仅 d≥256/多专家成立。**未报 T4 实测 wall-clock tokens/s（非 FLOPs）前不称成功**。
- **Theorem 2 单调性被削弱**：硬路由下仅活跃专家 F 被计算/更新，跳过专家 F 未定义。"F 单调" 降级为"每活跃块在其更新下 F 非增"——有意义（约束路由器不破坏已选专家收敛）但非原 Theorem 2。作独立可证陈述，不冒称原定理。
- **F-PC 线需 W_down**：重接 Theorem 2 但新增参数 + 顶层 mu 定义；F-inertial 线无新参但弱（非严格 PC）。
- **char/BPE LM 非 TextWorld**：仍是语言建模 NLL+速度。

### 实验目标（可证伪，不写代码）

| 门 | 比较/条件 | 主指标 | 判定 |
|---|---|---|---|
| G1 速度（go/no-go） | 块稀疏推理 wall-clock tokens/s vs dense d4 | ≥1.5× 且 BPC 相当 | T4 实测，非 FLOPs；FAIL 则记"仅大规模"有效负面边界 |
| G2 惰性衰减保真 | 惰性 `z=d^Δt z` vs 密集未跳过 | max abs diff <1e-5 | 精确；FAIL=实现 bug |
| G3 路由有效性 | q_φ argmin F̂ vs 真实 F argmin 一致率 | ≥80% 块 | 接近随机→F 头未学 |
| G4 F 整合 | F-PC 线：证明+测每活跃块 F 在其更新下非增 | 单调（经验） | Theorem-2 风格弱陈述 |
| G5 无崩溃 | soft→硬退火后专家使用熵 | ≥log E −1，每专家≥5% | 单专家>70%=崩溃 |
| G6 主线（条件于 G1） | FE-1 catgirl BPC + tokens/s vs Transformer | 超 Transformer | G1 成立才有意义 |

### F 双线对照实验

FE-1a（F-PC）与 FE-1b（F-inertial）独立跑 G1-G6，比：F̂ 预测精度（G3）、路由有效性、BPC、tokens/s、实现复杂度。判定哪条 F 线更值得进 FE-3。

### 结论 / 决定

- **采纳 FE-1 为首发切片**（用户确认逐个来）；FE-3 为目标态，FE-1 验证后再叠 FE-2。
- **F 双线都试**（用户确认）：FE-1a=F-PC（加 W_down，接 Theorem 2，修容量洞），FE-1b=F-inertial（无 W_down，惯性延续，便宜弱）。独立判官。
- **理论设计完成，不写代码**（用户要求）。待用户批准后进实现。
- **诚实标注**：T4 小模型速度风险（G1 go/no-go）、Theorem 2 弱化（G4）、两主线当前断裂需重定义 F。

### 待办（批准后）

1. 实现 FE-1b（F-inertial，无 W_down，最简）先跑 G1/G2/G3——最快验证路由+惰性衰减是否真省 wall-clock。
2. 实现 FE-1a（F-PC，加 W_down）跑 G1-G4——验证 Theorem 2 重接。
3. G1 通过则跑 G6（catgirl vs Transformer）；G1 FAIL 记负面边界。

### 可复现信息

- 本条为理论设计 + 预注册，无运行。源码核查：`vpsc/world_model/scaling_variants.py:114`（d4 dense）、`vpsc/world_model/portable_gated_trace.py:_gated_trace`（惰性衰减）、`vpsc/world_model/cores.py:3117`（稀疏 query）、`vpsc/free_energy.py`+`vpsc/network.py:183`（早期 F 线，未接 E3）。
- 关联：SG29（d4 质量超 Transformer 但不省算力）、S1（W_down 容量洞，F-PC 线修复）、Theorem 2（F 单调，弱化为块级）、研究主线（SNN 降算力 + 超 Transformer——FE-1 为算力方向）。
