...
- 正式矩阵执行前，以上协议、门槛、static dispatcher 与 verdict 已由 commit `f67aeef`（`:test_tube: feat(research): 冻结损失密度算子矩阵`）提交并推送；结果 JSON 记录的 clean-tree commit 同为 `f67aeefea7d25744373ce54d0fbc24e6ba5452a8`。
- 正式命令与冻结命令完全一致：`python3 experiments/e3_ldaa1_operator_crossover.py --device cpu --lengths 512 2048 8192 32768 --loss-densities 0.0009765625 0.00390625 0.015625 0.0625 0.25 1.0 --state-dims 16 64 --input-gradients off on --threads 4 --warmup 1 --repeats 5 --out results/e3_scan/e3_ldaa1_operator_crossover.json`。
- 环境：macOS arm64，Python `3.9.6`，PyTorch `2.8.0`，CPU，PyTorch intra-op threads=`4`；全矩阵 wall 约 `120s`（外部轮询计时，仅作资源记录，不参与判官）。
- 产物：`results/e3_scan/e3_ldaa1_operator_crossover.json`，`720,969 bytes`，SHA-256 `a555dbbb010892b7a7db1e508ed6a7bf3be8a6b45d0120c8f4e24f8f15c5fc6e`。

### 正式 gate 结果

| 冻结判官 | 要求 | 结果 | 判定 |
|---|---:|---:|---|
| H1 exactness | 96 cells、全部 exact backend 通过 `atol=2e-5, rtol=1e-4` | `96/96`；forward eligibility / reverse / segmented 的全局最大绝对误差均不超过 `1.1920929e-6` | **PASS** |
| H2 sparse crossover | sparse cells 中组合门通过率 `>=60%` | `48/48 = 100%` | **PASS** |
| H2 input-gradient audit | input-on sparse cells 中组合门通过率 `>=60%` | `24/24 = 100%` | **PASS** |
| RQ2 frozen static dispatcher | 每 cell 延迟 `<=1.10x` 四后端 oracle | `85/96 = 88.54%`；mean/p95/max regret=`1.030/1.184/1.408x` | **FAIL** |

矩阵共 `96` cells、每 cell 四后端，即 `384` 组 backend 记录；未出现非有限值、资源失败或 exactness 失败。冻结 sparse 区的 segmented 指标如下：

| 指标 | min | mean | max |
|---|---:|---:|---:|
| BPTT p50 / segmented p50 | `2.120x` | `3.987x` | `6.727x` |
| segmented unique storage / BPTT | `9.81%` | `12.28%` | `15.42%` |
| input-on：BPTT p50 / segmented p50 | `2.132x` | `3.967x` | `6.222x` |
| dense reverse p50 / segmented p50 | `0.918x` | `1.146x` | `1.405x` |

四后端 oracle 次数为：BPTT `0`、forward eligibility `3`、dense reverse `34`、segmented `59`。static dispatcher 的 11 个超门 cell 全部位于规则选择 dense reverse、但实测 oracle 为 segmented 的区域；这证明 actual density 单阈值不足以完成 RQ2，但不能反向修改本轮阈值。

### 观察与解释

- **观察**：segmented backend 在保留完整逐时刻参数梯度、并在 `input_gradient=on` 时返回完整输入梯度的条件下，96/96 cells 与 BPTT 数值一致。因此收益不是删除 dense gradient 或改变 loss 得到的近似。
- **观察**：冻结 sparse 区 48/48 cells 同时满足 `>=1.5x BPTT` 与 `<=25% BPTT unique storage`，且 input-on 子集 24/24 仍成立。这是 LDAA 的 operator-level 强正面结果。
- **边界**：segmented 并非普遍快于 dense reverse；其 sparse speedup 最低为 `0.918x`。可成立的主张是“相对 BPTT 存在稳定 sparse crossover”，而不是“segmented 对所有 exact scan 占优”。
- **解释**：冻结 dispatcher 只看 density，遗漏了 T、state dimension、input-gradient 与常数开销的交互；11 个失败 cell 说明 RQ2 需要只使用运行前特征、带训练/留出划分的 selector，不能按本轮 oracle 标签手调阈值并追认成功。
- **外推限制**：本轮是 macOS CPU operator benchmark；没有 CUDA segmented kernel，也没有真实语言/世界模型 update，更没有第二个非 VPSC diagonal recurrent/SSM core。不能由此声称 H3、GPU crossover 或端到端质量成立。

### 结论 / 决定

- 机器 verdict 原样为 **`OPERATOR_GO_MODEL_VALIDATION_REQUIRED`**：H1、H2 overall 与 H2 input-on 全通过，允许进入 LDAA-2；不进入 `main`。
- static dispatcher gate **FAIL**，RQ2 仍未完成；保留该负诊断，不以 post-hoc 阈值覆盖。
- LDAA-2 必须冻结并验证：至少一个非 VPSC diagonal recurrence、真实模型 update 与 held-out quality；dispatcher 只可用运行前特征，在训练/留出 cell 上评估 `<=1.10x oracle`。这些完成前不得把 operator 正面结果升级为模型级或通用系统主张。
