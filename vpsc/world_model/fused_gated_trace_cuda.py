"""Lazy loader and autograd binding for the SG25C native CUDA kernel."""
...
SOURCES = (
    SOURCE_DIR / "sg25c_gated_trace.cpp",
    SOURCE_DIR / "sg25c_gated_trace_kernel.cu",
)
_EXTENSION: Any = None
...
def load_extension(*, verbose: bool = False) -> Tuple[Any, Dict[str, Any]]:
    ...
    name = f"vpsc_sg25c_{fingerprint[:12]}"
    default_build = Path("/root/autodl-tmp/torch_extensions/sg25c")
    build_directory = Path(
        os.environ.get("VPSC_SG25C_BUILD_DIR", str(default_build))
    )
...
class _FusedGatedTrace(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx: Any,
        drives: torch.Tensor,
        ...
        if not drives.is_cuda or drives.dtype != torch.float32:
            raise TypeError("SG25C fused trace requires CUDA float32 drives")
