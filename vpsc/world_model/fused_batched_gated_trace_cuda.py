"""Lazy CUDA binding for SG25E per-example batched trace queries."""
...
SOURCES = (
    SOURCE_DIR / "sg25e_batched_gated_trace.cpp",
    SOURCE_DIR / "sg25e_batched_gated_trace_kernel.cu",
)
_EXTENSION: Any = None
...
def load_extension(*, verbose: bool = False) -> Tuple[Any, Dict[str, Any]]:
    ...
    name = f"vpsc_sg25e_{fingerprint[:12]}"
    default_build = Path("/root/autodl-tmp/torch_extensions/sg25e")
    build_directory = Path(
        os.environ.get("VPSC_SG25E_BUILD_DIR", str(default_build))
    )
    build_directory.mkdir(parents=True, exist_ok=True)
    executable_directory = Path(sys.executable).parent
    bundled_ninja = executable_directory / "ninja"
...
class _FusedBatchedGatedTrace(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx: Any,
        drives: torch.Tensor,
        ...
        if not drives.is_cuda or drives.dtype != torch.float32:
            raise TypeError("SG25E fused trace requires CUDA float32 drives")
        if query_indices.ndim != 2 or query_indices.dtype != torch.long:
            raise TypeError("SG25E query_indices must be int64 [batch,query]")
        ...
        ctx.save_for_backward(
            drives, query_indices, decays, previous, writes, raw
        )
    ...
    def backward(
        ...
        drives, query_indices, decays, previous, writes, raw = ctx.saved_tensors
        if grad_raw is None:
            grad_raw = torch.zeros_like(raw)
        if grad_final_e is None:
            grad_final_e = torch.zeros_like(previous[:, 0, : decays.shape[1]])
        if grad_final_i is None:
            grad_final_i = torch.zeros_like(previous[:, 0, decays.shape[1] :])
        extension, _ = load_extension()
        grad_drives, grad_decays, grad_initial_e, grad_initial_i = (
            extension.backward(
                grad_raw.contiguous(),
...
def fused_batched_gated_trace(
    ...
    *,
    ...
    return _FusedBatchedGatedTrace.apply(
        drives,
        query_indices,
        decays,
        initial_e,
