...
__global__ void gated_trace_forward_kernel(
  ...
  for (int time_index = 0; time_index < time_steps; ++time_index) {
    ...
    const float content_e = drives[drive_base + state_index] >= 0.0f ? 1.0f : 0.0f;
    ...
    if (query_cursor < query_count && query_indices[query_cursor] == time_index) {
      const int raw_base =
          (batch_index * query_count + query_cursor) * (4 * state_dim);
      raw[raw_base + state_index] = trace_e >= spike_threshold ? 1.0f : 0.0f;
...
__global__ void gated_trace_backward_kernel(
  ...
  int query_cursor = query_count - 1;

  for (int time_index = time_steps - 1; time_index >= 0; --time_index) {
    float direct_e = 0.0f;
    float direct_i = 0.0f;
    if (query_cursor >= 0 && query_indices[query_cursor] == time_index) {
      const int raw_base =
          (batch_index * query_count + query_cursor) * (4 * state_dim);
      ...
      const float spike_signal_i = -grad_raw[raw_base + state_dim + state_index];
      const float trace_signal_e = grad_raw[raw_base + 2 * state_dim + state_index];
      const float trace_signal_i = -grad_raw[raw_base + 3 * state_dim + state_index];
...
std::vector<torch::Tensor> sg25c_gated_trace_forward_cuda(
  ...
  TORCH_CHECK(query_indices.scalar_type() == at::kLong, "query_indices must be int64");
  ...
  TORCH_CHECK(decays.dim() == 2 && decays.size(0) == 2, "decays must be [2,state]");
  TORCH_CHECK(query_indices.dim() == 1 && query_indices.numel() > 0,
              "query_indices must be non-empty 1D");
  const int64_t batch = drives.size(0);
  const int64_t time_steps = drives.size(1);
  const int64_t state_dim = decays.size(1);
  const int64_t query_count = query_indices.numel();
  ...
  auto raw = torch::empty({batch, query_count, 4 * state_dim}, drives.options());
  auto final_e = torch::empty({batch, state_dim}, drives.options());
  auto final_i = torch::empty({batch, state_dim}, drives.options());
  auto previous = torch::empty({batch, time_steps, 2 * state_dim}, drives.options());
  auto writes = torch::empty_like(previous);
  const int threads = 128;
  const int blocks = static_cast<int>((batch * state_dim + threads - 1) / threads);
  gated_trace_forward_kernel<<<
