"""Binary input-event exact-reset scan A0 and ANN-speed gates."""
...
def _core_models(seed: int, cache_tokens: int) -> Dict[str, TemporalCore[Any]]:
    torch.manual_seed(seed)
    return {
        "ic0": E3InputCodedScanCore(32, 32, state_dim=STATE_DIM),
...
def build_a0_models(seed: int) -> Dict[str, DiagnosticModel]:
    ...
    cores = _core_models(seed + 1000, 32)
    models = {}
    for model in cores.values():
        wrapper = DiagnosticModel(model)
        with torch.no_grad():
            wrapper.embedding.weight.copy_(shared["embedding.weight"])
            wrapper.output_norm.weight.copy_(shared["output_norm.weight"])
            wrapper.output_norm.bias.copy_(shared["output_norm.bias"])
            wrapper.decoder.weight.copy_(shared["decoder.weight"])
            wrapper.decoder.bias.copy_(shared["decoder.bias"])
        name = "ic0" if isinstance(model, E3InputCodedScanCore) else (
            "lstm" if isinstance(model, StatefulLSTMCore) else "transformer"
        )
        models[name] = wrapper
    return models
...
def run_a0() -> Dict[str, Any]:
    ...
    if any(abs(value - lstm) / lstm > 0.02 for value in counts.values()):
        raise AssertionError(f"IC0 parameter fairness failed: {counts}")
    results = {}
    for name in ("ic0", "lstm", "transformer"):
        train = train_model(
...
def benchmark_training(
    *, threads: Sequence[int], warmup: int, repeats: int
    ...
    for thread_count in threads:
        ...
        cores = {
            name: core.train(True)
            for name, core in _core_models(8_200_000 + thread_count, 512).items()
        }
        base = torch.randn(1, 512, 32)
...
def benchmark_streaming(
    *, threads: Sequence[int], warmup_steps: int, measured_steps: int
) -> Sequence[Dict[str, Any]]:
    records = []
    for thread_count in threads:
        torch.set_num_threads(thread_count)
        cores = {
            name: core.eval()
            for name, core in _core_models(8_400_000 + thread_count, 512).items()
        }
        tokens = torch.randn(warmup_steps + measured_steps, 1, 32)
...
def _decision(a0: Dict[str, Any], training: Sequence[Dict[str, Any]], streaming: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for record in training:
        ...
        ann[key] = {
            "train_ic0_ms": record["models"]["ic0"]["p50_ms"],
            "train_lstm_ms": record["models"]["lstm"]["p50_ms"],
            "stream_ic0_p95_ms": stream["models"]["ic0"]["p95_ms"],
            "stream_lstm_p95_ms": stream["models"]["lstm"]["p95_ms"],
        }
        ann[key]["passed"] = (
            ann[key]["train_ic0_ms"] <= ann[key]["train_lstm_ms"]
            and ann[key]["stream_ic0_p95_ms"] <= ann[key]["stream_lstm_p95_ms"]
        )
    return {
        "a0_gate": "PASS" if a0["passed"] else "FAIL",
        "ann_train_and_inference_speed_gate": (
            "PASS" if any(value["passed"] for value in ann.values()) else "FAIL"
        ),
        "ann_checks": ann,
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output", type=Path, default=Path("results/e3_scan/e3_ic0_input_code.json")
    )
...
def main() -> None:
    ...
    training = benchmark_training(
        threads=threads, warmup=args.warmup, repeats=args.repeats
    )
    ...
    result = {
        "schema_version": 1,
        "experiment": "E3-IC0 binary input-event exact reset scan",
